...
class Solution {
    public long countValidNumbers(int idx, int tight, String numStr, String suffix, int limit, int numLength,
            long[][] dp) {
        if (idx == numLength)
            return 1;
        if (dp[idx][tight] != -1)
        ...
        for (int i = (int) lowerBound; i <= upperBound; i++)
            count += countValidNumbers(idx + 1, (tight == 1 && i == numStr.charAt(idx) - '0') ? 1 : 0, numStr, suffix,
                    limit, numLength, dp);
        dp[idx][tight] = count;
        return count;
    }
